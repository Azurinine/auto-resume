# Project Constitution & System Manual — Gemini
...
*   **Build Pipeline** ⚡: To compile the tailored resume, the AI agent (Gemini) must run the following exact command in the terminal: `export PATH=$PATH:/Library/TeX/texbin && make NAME=company_name-job_posting-date`. This ensures `latexmk` is found in the PATH and the output file is named correctly.
    * **Crucial Agent Instruction:** The agent must run this as a single combined command using the `run_command` tool. Do **not** use the `ask_permission` tool first; simply execute the `run_command` directly.
...
*   **Error Handling**: In the event of a build failure, the AI agent (Gemini) must check the `output/main.log` file first to accurately diagnose the root cause before proposing any solutions.
